...
model_spillover_2 = lmer(residuals ~ logtrial * ctr_nameCategory * ctr_consistency +
                           (1 + (ctr_nameCategory * ctr_consistency) | trial_id) +
                           (1 + (ctr_nameCategory * ctr_consistency) | workerid),
                         spilloverdf, REML = F)
summary(model_spillover_2)

model_spillover_2_logctr = lmer(residuals ~ ctr_logtrial * ctr_nameCategory * ctr_consistency +
                           (1 + (ctr_nameCategory * ctr_consistency) | trial_id) +
                           (1 + (ctr_nameCategory * ctr_consistency) | workerid),
                         spilloverdf, REML = F)
summary(model_spillover_2_logctr)
...
## 10.24 analyses: by-worker and spillover 3
### by-worker variation (unfaceted)
residuals_byname_byworker = subregion_df %>%
  group_by(region, consistency, nameCategory, workerid) %>% 
  summarize(mean_resid = mean(residuals), CILow=ci.low(residuals), CIHigh=ci.high(residuals)) %>% 
  ungroup() %>% 
  mutate(YMin=mean_resid-CILow,YMax=mean_resid+CIHigh)


residuals_byname_byworker$region <- factor(residuals_byname_byworker$region,
                                           levels=c("pre_crit_2",
                                                    "pre_crit_1",
                                                    "crit",
                                                    "spillover_1",
                                                    ...
                                                    "spillover_3",
                                                    "spillover_4"))
residuals_byname_byworker$nameCategory <- recode(residuals_byname_byworker$nameCategory, 
                                                 "black" = "Black-normed Names",
                                                 "white" = "White-normed Names")

ggplot(residuals_byname_byworker, aes(x=region, y=mean_resid, color=consistency, group=consistency)) + 
  facet_wrap(nameCategory~workerid) +
  geom_point()  + 
  geom_errorbar(aes(ymin=YMin,ymax=YMax),width=.25) +
  geom_line() +
  scale_colour_manual("Stereotype\nConsistency", values = c("#00397A","#CCCCCC")) +
...
### by-worker variation (with ordering effects)
#going to be really hard to do and analyze in one visualization

### spillover 3: under the microscope
...
#### but what about spillovers 2 and 1?
#spill2
